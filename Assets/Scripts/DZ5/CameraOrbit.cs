...
namespace NikolayTrofimovDZ5
{
    public class CameraOrbit : MonoBehaviour
    {
        ...
        [SerializeField, Min(0)] private float _focusRadius = 1.0f;
        [SerializeField, Range(0, 1.0f)] private float _focusCentering = 0.5f;
        [SerializeField, Range(0.1f, 5.0f)] private float _sensetive = 0.5f;
        ...
        private Vector2 _orbitAngles = new Vector2(45.0f, 0);
        ...
        public void CameraMovement()
        {
            ...
            Vector3 lookDirrection = lookRotation * Vector3.forward;
            LookPosition = _focusPoint + lookDirrection;
            if(Physics.BoxCast(_focusPoint, CameraHalfExtends, -lookDirrection, 
                out RaycastHit hit, lookRotation, _distance - _regularCamera.nearClipPlane, _obstacleMask))
            {
            ...
            Vector3 lookPosition = _focusPoint - lookDirrection * _currentDistance;
        ...
        {
            Vector2 intut = new Vector2(-Input.GetAxis("Mouse Y"), Input.GetAxis("Mouse X"));
            float e = Mathf.Epsilon;
            if(intut.x < -e || intut.x > e || intut.y < -e || intut.y > e)
            {
                orbitAngles += _rotationSpeed * _sensetive * Time.unscaledDeltaTime * intut;
                return true;
            }
            return false;
        }
        ...
        private static float GetAngles(Vector2 dirrection)
        {
            float angle = Mathf.Acos(dirrection.y) * Mathf.Deg2Rad;
            return dirrection.x < 0 ? 360.0f - angle : angle;
        }
    }
}